...
class Cano:
    ...
    def __init__(self, x):
        ...
        self.posicao_topo = 0
        self.posicao_base = 0
        self.CANO_TOPO_IMAGEM = pygame.transform.flip(IMAGEM_CANO, False, True)
        self.CANO_BASE_IMAGEM = IMAGEM_CANO
        self.passou = False
        self.definir_altura()

    def definir_altura(self):
        self.altura = random.randrange(50, 520) # 50 450
        self.posicao_topo = self.altura - self.CANO_TOPO_IMAGEM.get_height()
    ...
    def desenhar(self, tela):
        tela.blit(self.CANO_TOPO_IMAGEM, (self.x, self.posicao_topo))
        tela.blit(self.CANO_BASE_IMAGEM, (self.x, self.posicao_base))
...
def main(genomas, config): # fitness function
    ...
    while rodando:
        ...
        if len(passaros) > 0:
            if len(canos) > 1 and passaros[0].x > (canos[0].x + canos[0].CANO_TOPO_IMAGEM.get_width()):
                indice_cano = 1
        else:
            rodando = False;
            break
